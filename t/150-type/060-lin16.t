...
Imager->open_log(log => "testout/linimg16.log");
...
my $im_g = Imager::i_lin_img_16_new(100, 101, 1);
...
my $im_rgb = Imager::i_lin_img_16_new(100, 101, 3);
...
# basic OO tests
my $oo16img = Imager->new(xsize=>200, ysize=>201, bits=>16, linear => 1);
ok($oo16img, "make a 16-bit oo image");
check_vtable($oo16img, "16-bit linear vtable");
...
ok(!Imager->new(xsize=>0, ysize=>1, bits=>16, linear => 1),
...
ok(!Imager->new(xsize=>-1, ysize=>1, bits=>16, linear => 1),
...
ok(!Imager->new(xsize=>1, ysize=>1, bits=>16, channels=>0, linear => 1),
    "fail to create a zero channel image");
cmp_ok(Imager->errstr, '=~', qr/there must be extra channels if channels is zero/,
       "and correct error message");
ok(!Imager->new(xsize=>1, ysize=>1, bits=>16, channels=>5, linear => 1),
...

{
 ...
 SKIP: {
    ...
    my $im_b = Imager->new(xsize=>$dim1, ysize=>$dim1, channels=>1, bits=>16, linear => 1);
    is($im_b, undef, "integer overflow check - 1 channel");
    
    $im_b = Imager->new(xisze=>$dim1, ysize=>1, channels=>1, bits=>16, linear => 1);
    ok($im_b, "but same width ok");
    $im_b = Imager->new(xisze=>1, ysize=>$dim1, channels=>1, bits=>16, linear => 1);
    ...
    $im_b = Imager->new(xsize=>$dim3, ysize=>$dim3, channels=>3, bits=>16, linear => 1);
    is($im_b, undef, "integer overflow check - 3 channel");
    
    $im_b = Imager->new(xisze=>$dim3, ysize=>1, channels=>3, bits=>16, linear => 1);
    ok($im_b, "but same width ok");
    $im_b = Imager->new(xisze=>1, ysize=>$dim3, channels=>3, bits=>16, linear => 1);
    ok($im_b, "but same height ok");
    ...
    my $im_o = Imager->new(xsize=>$dim4, ysize=>1, channels=>1, bits=>16, linear => 1);
...
{ # check the channel mask function
  ...
  my $im = Imager->new(xsize => 10, ysize=>10, bits=>16, linear => 1);
...
{ # convert to rgb16
  my $im = test_image();
  my $im16 = $im->to_linear(bits => 16);
  print "# check conversion to 16 bit linear\n";
  is($im16->bits, 16, "check bits");
  ok($im16->linear, "check it's linear");
...
{ # empty image handling
  ...
  ok(!$im->to_linear(bits => 16), "convert empty image to 16-bit linear");
...
{
  # extra channels
  my $im = Imager->new(xsize => 10, ysize => 11, channels => 3, extrachannels => 5, linear => 1, bits => 16);
...
if (0) { # FIXME?
  my $im = Imager->new(xsize => 10, ysize => 10, bits => 16, channels => 3, linear => 1);
...
{ # psamp
  print "# psamp\n";
  my $imraw = Imager::i_lin_img_16_new(10, 10, 3);
  {
...
std_image_tests({ bits => 16, linear => 1 });
