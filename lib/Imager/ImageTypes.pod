...
8-bit/sample RGB image, 24 bits are stored for each pixel.  A direct
color image can have 0 to 4 color channels and up to 10 extra
channels.

=item *

paletted - an index into a table of colors is stored for each pixel.
A paletted image has 1 to 4 channels.

=back

Imager treats these as follows:

=over

=item *

0 samples per color - the image has a color model not supported by
Imager.  The image must have L<extra channels|/Extra channels>.
...
Direct color images can also include "extra channels" which either
represent data associated with each pixel such as with TIFF extra
samples, or if the number of color channels is zero, may represent a
color model not supported by Imager, such as CMYK or YCbCr.

Extra channels are generally not preserved across transformations such
as scaling.

Support for extra channels is experimental, ongoing and subject to
change.
...
C<float> - one C float per sample.
...
=item channels()
X<channels() method>X<methods, channels()>

  print "Image has ",$img->getchannels(), " channels\n";
  print "Image has ",$img->channels(), " channels\n";
...
channels() is an alias for getchannels().

Returns an empty list if the image object is not initialized.

=item extrachannels()
X<extrachannels() method>X<methods, extrachannels()>

  print "Image has ", $img->extrachannels, " extra channels\n";

Returns the number of "extra" non-color channels defined for the
image.

Returns an empty list if the image object is not initialized.

=item totalchannels()
X<totalchannels() method>X<methods, totalchannels()>

  print "Image has ", $img->totalchannels, " total channels\n";

Returns the total number of channels defined for the
image, including both color and extra channels.
...
channel in a pixel, 8 for a normal image, 16 for 16-bit/sample image,
C<float> for float/sample images and C<double> for a double/sample
image.
...
Returns a true value if image samples are natively stored using a
linear tone curve, or false if the samples use the tone curve from the
current color profile, which is always sRGB at this point.  Note that
these samples are converted to tone curve scaled samples in most
cases.
...
Convert a paletted image (or any image) to an float/channel direct
color image.

  $rgbimg = $img->to_rgb_float;

No parameters.

=item to_linear()

Create a linear sample version of an image.
...
C<bits> - the bits per sample to use for the linear-sample image.  If
this source image has bits < 16 this defaults to 16 bits, otherwise it
defaults to the number of bits in the source image.

At this point only 16-bit/sample and double/sample linear images
exist.
...
=head1 Extra channels

Extra channels are a new feature intended for future use in:

=over

=item *

supporting extra samples that an image format might support in
addition to the color channels, such as extra samples in TIFF, and
extra alpha planes in some other image formats.

=item *

support for non-color images, including planes of scientific data,
where the image base channels would be zero.

=item *

support for raw non-RGB samples from a file.

=back

=for stopwords ABI

The current limited implementation is just to avoid another ABI break,
future releases of Imager should have better support.
