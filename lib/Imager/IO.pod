...
Returns the size of the stream, if it's a file or memory buffer.
...
=item get_max_mmap_size()

  my $size = Imager::IO->get_max_mmap_size();

Fetch the maximum allowed address space than can be used to map a file
into memory.

=item set_max_mmap_size()

  my $ok = Imager::IO->get_max_mmap_size($new_size);

Set the maximum allowed address space than can be used to map a file
into memory.
