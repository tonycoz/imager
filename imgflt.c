...
static i_img_dim i_gsamp_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_sample_t *samps, 
                       int const *chans, int chan_count);
static i_img_dim i_gsampf_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_fsample_t *samps, 
                        int const *chans, int chan_count);
static i_img_dim 
i_psamp_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, const i_sample_t *samps, const int *chans, int chan_count);
static i_img_dim 
i_psampf_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, const i_fsample_t *samps, const int *chans, int chan_count);
static i_image_data_alloc_t *
i_data_float(i_img *im, i_data_layout_t layout, i_img_bits_t bits, unsigned flags,
              void **pdata, size_t *psize, int *pextra);
static i_img_dim i_gslinflt_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_sample16_t *samps, const int *chans, int chan_count);
static i_img_dim i_gslinfltf_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_fsample_t *samps, const int *chans, int chan_count);
static i_img_dim i_pslinflt_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, const i_sample16_t *samps, const int *chans, int chan_count);
static i_img_dim i_pslinfltf_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, const i_fsample_t *samps, const int *chans, int chan_count);
...
vtable_float = {
  ...
  i_gsamp_dflt, /* i_f_gsamp */
  i_gsampf_dflt, /* i_f_gsampf */

  i_psamp_dflt, /* i_f_psamp */
  i_psampf_dflt, /* i_f_psampf */
...
i_img *
im_img_float_new_extra(pIMCTX, i_img_dim x, i_img_dim y, int ch, int extra) {
  size_t bytes;
  i_img *im;
  int totalch;
  ...
  totalch = ch + extra;
  bytes = (i_img_dim_u)x * (i_img_dim_u)y * (i_img_dim_u)totalch * sizeof(float);
  if (bytes / y / totalch / sizeof(float) != x) {
    im_push_errorf(aIMCTX, 0, "integer overflow calculating image allocation");
    return NULL;
  }
  
  im = im_img_new(aIMCTX, &vtable_float, x, y, ch, extra, i_float_bits);
  if (im == NULL)
    return NULL;
...
static i_img_dim
i_gsamp_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_sample_t *samps, 
             int const *chans, int chan_count) {
  int ch;
  i_img_dim count, i, w;
  ...
  if (y >=0 && y < im->ysize && l < im->xsize && l >= 0) {
    ...
    if (chans) {
      ...
      for (i = 0; i < w; ++i) {
        for (ch = 0; ch < chan_count; ++ch) {
          *samps++ = SampleFTo8(data[off+chans[ch]]);
          ++count;
        }
        off += totalch;
      }
...
static i_img_dim
i_gsampf_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, i_fsample_t *samps, 
               int const *chans, int chan_count) {
  if (y >=0 && y < im->ysize && l < im->xsize && l >= 0) {
    int ch;
    i_img_dim count, i, w;
...
static
i_img_dim
i_psamp_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, 
  ...
  if (y >=0 && y < im->ysize && l < im->xsize && l >= 0) {
	...
	for (i = 0; i < w; ++i) {
	  for (ch = 0; ch < chan_count; ++ch) {
	    data[offset + chans[ch]] = Sample8ToF(*samps);
	    ++samps;
	    ++count;
	  }
	  offset += totalch;
	...
	unsigned mask = 1;
        for (ch = 0; ch < chan_count; ++ch) {
	  if (im->ch_mask & mask)
	    data[offset + ch] = Sample8ToF(*samps);
...
static
i_img_dim
i_psampf_dflt(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, 
...
static i_img_dim
i_gslinflt_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y,
  ...
  if (y >=0 && y < im->ysize && l < im->xsize && l >= 0) {
    ...
    if (chans) {
	  ...
	  int ch = chans[chi];
	  i_fsample_t raw = data[off+chans[ch]];
	  if (ch < color_chans)
...
static i_img_dim
i_gslinfltf_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y,
...
static
i_img_dim
i_pslinflt_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, 
...
static
i_img_dim
i_pslinfltf_d(i_img *im, i_img_dim l, i_img_dim r, i_img_dim y, 
	  const i_fsample_t *samps, const int *chans, int chan_count) {
  dIMCTXim(im);
  i_img_dim count, i, w;
  int color_chans;
